...
**استپ‌های تمام‌شده:** ۱ تا ۲۰ ✅ _(استپ ۱۶ زودتر از ۱۲ انجام شد چون استپ ۱۲ به آن وابسته بود — طبق AGENT_RULE 3)_ + اصلاح کوچک hotfix: محدودکردن هشدار `API_KEYS_ENABLED` فقط به حالت `multi` (کامیت `8b963e8`).

**🚀 فاز بزرگ بعدی (استپ ۲۱–۳۰) — پلتفرم اتوماسیون نودبیس در شأن n8n:** نقشهٔ کامل در انتهای `PLAN.md` ثبت شد. خلاصه:
- **مدل اجرا:** n8n = **مدل ب** (ورکفلوی ذخیره‌شده + Trigger)؛ افزونهٔ کروم = **مدل الف** (افزونه = پنل، با لاگین وارد سرور، انتخاب مستقیم المان روی مرورگر).
- **کشف کلیدی:** بک‌اند **خیلی جلوتر از UI** است. از قبل آماده: موتور workflow کامل (`if/while/loop/foreach/switch/try/...` در `pipeline.ts`)، رویدادهای لحظه‌ای هر استپ (`step.start/done/error` با durationMs و error از طریق `onEvent`→`LiveBus`)، کانال زندهٔ WS/SSE، ذخیرهٔ چند-ورکفلو + نسخه‌بندی (`WorkflowService` + `/workflows/:userId` CRUD + `/versions` + `/run`)، webhook خروجی HMAC، نود n8n + Trigger node، افزونهٔ پایه. **شکاف فقط UI است.**
- **ترتیب استپ‌ها:** ۲۱ کتابخانهٔ چند-ورکفلو در UI (اتصال به CRUD موجود) → ۲۲ لاگ زنده + نتیجهٔ هر نود مثل n8n → ۲۳ بازطراحی بصری ادیتور (drag&drop/شاخه‌دار) → ۲۴ فیلدهای غنی (checkbox/code/json/list/expression) → ۲۵ Triggerها (Webhook/Schedule/Telegram) → ۲۶ گزارش دوکاناله (webhook هر استپ + لینک زنده) → ۲۷ نود n8n غنی → ۲۸ افزونهٔ کروم=پنل + element picker + تیک/خطا روی نود → ۲۹ همگام‌سازی کاتالوگ اکشن → ۳۰ جمع‌بندی/مستندسازی.
- **لایسنس:** از Automa (AGPL + لایسنس تجاری) و n8n فقط **الهام UX** گرفته می‌شود؛ **هیچ کدی کپی نمی‌شود** — بازسازی با استک خالص خودمان (vanilla JS، CSP-safe).

**استپ بعدی برای شروع:** **استپ ۲۱ — کتابخانهٔ چند-ورکفلو در UI** (منتظر تأیید صاحب پروژه برای شروع). ⚠️ همان قرارداد CRLF برای `src/**` (+`package.json` ریشه + `.env.example` + `ecosystem.config.js`) و LF برای `tests/**`/`docs/**`/`public/**`/`extension/**`/`n8n-node/**`/`install.sh`/`.github/**` رعایت شود (ویرایش `src/**` با اسکریپت پایتونِ binary-mode).
