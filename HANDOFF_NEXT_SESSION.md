...
>
> ✅ **مرجع اصلی ماموریت‌ها: `MISSIONS.md` (ریشه‌ی مخزن).** فهرست تیک‌دارِ همه‌ی
> خواسته‌های مالک پروژه، وضعیت هر کدام، و نقشه‌ی پیاده‌سازیِ آیتم‌های باقی‌مانده
> آنجاست. **قاعده‌ی ثابت R1:** هنگام تنظیم آپشن‌های هر نود، حتماً منطق نودهای
> پروژه‌ی [automa](https://github.com/automaapp/automa) مرجع گرفته شود.
>
...
>
> 🚩 **آخرین وضعیت (2026-07-30) — از اینجا شروع کن:**
> **`docs/uiux/09-HANDOFF-item-N-per-node-run.md`**
> آیتم‌های A تا M تمام شده‌اند (H/J/I در PR #11:
> https://github.com/jalil-ahmadi2/plyr/pull/11). تنها آیتم باز، **N (اجرای
> تک‌نود)** است و **مشخصات کامل پیاده‌سازی‌اش** (قرارداد اندپوینت
> `POST /run-node`، اسکیمای zod، سیم‌کشی فرانت، کلیدهای i18n، و تست‌هایی که باید
> نوشته شود) با شماره‌ی خط در همان فایل بخش ۲ آمده است. بخش ۳ همان فایل **دو باگ
> واقعی** را ثبت کرده: ناهم‌ترازی «ایندکس زنجیره» با «ایندکس استپ» وقتی نودی
> `disabled` است، و دکمهٔ «Run node» در NDV که امروز **کل فلو** را اجرا می‌کند
> (نقض قانون «UI دروغین»). ترتیب خواندن: `09-…` → `08-…` (چیزهایی که شیپ شده و
> فهرست تله‌ها) → `07-…` §۴ و §۵.۶.
>
> 🗂 سند قدیمی‌تر (فقط برای تاریخچه):
