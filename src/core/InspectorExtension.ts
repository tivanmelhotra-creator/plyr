...
/**
 * The address the remote browser should use to reach this server.
 *
 * Loopback, deliberately. The remote Chromium runs on this same host, so
 ...
 *
 * This copy of the extension is side-loaded into the REMOTE browser by the
 * server, so it is told where that server is instead of asking the user to type
 * it in from inside a remote desktop. Applied as DEFAULTS only: a value the user
 * has saved is never overwritten (see applyBootstrapDefaults in background.js).
 */
'use strict';
var AB_BOOTSTRAP = {
  baseUrl: ${base},
