...
این راهنما جریان کامل کار را از ساختِ ورکفلو تا اجرای زنده توضیح می‌دهد و نقاط ورود (entry point) به موتور یکسانِ سرور را به هم وصل می‌کند:

1. **سرور / پنل وب** — Builder خطی و Visual Editor در `public/`.
2. **افزونهٔ کروم + نمای زنده** — کلاینت سبک (Model A) که فقط یک درخواست می‌فرستد و نتیجه را زنده آینه می‌کند.
3. **هر کلاینت HTTP دیگر** — با همان API عمومی (`POST /run`، `POST /workflows/:u/:w/run`) از اسکریپت، زمان‌بند یا سرویس بیرونی.

> یک حقیقت مهم: **همهٔ این مسیرها روی یک موتور واحد اجرا می‌شوند** — `src/pipeline.ts` روی سرور. هیچ‌کدام از کلاینت‌ها (افزونه/پنل/کلاینت API) خودشان مرورگر را راه نمی‌اندازند؛ آن‌ها فقط درخواست می‌فرستند و رویدادهای زنده را نمایش می‌دهند.
...
```
   ...
   ┌──────────┐        │                                              │
   │ کلاینت   │──POST──▶│  POST /run            (اجرای inline)         │
   │   API    │        │  POST /workflows/:u/:w/run (اجرای نسخهٔ ذخیره)│
   └──────────┘        │                                              │
                       │  Worker ◀── صف ◀── جاب                        │
   ┌──────────┐        │     │                                        │
...
## ۲. ساخت ورکفلو (چند راه، یک خروجی)

همهٔ مسیرها به یک مدل دادهٔ یکسان می‌رسند: آرایه‌ای از استپ‌ها به شکل
`{ action, params }`. مدل دادهٔ بین استپ‌ها هم آیتم‌محور است:
`[{ json, binary? }]`.
...
## ۶. کلاینت‌های HTTP بیرونی

هر کلاینت HTTP همان دو مدل را پوشش می‌دهد: می‌توانید استپ‌ها را inline بفرستید
(`POST /run`) یا یک ورکفلوی ذخیره‌شده را با `workflowId` اجرا کنید
(`POST /workflows/:userId/:workflowId/run`). خروجی به‌صورت آیتم‌های
`[{ json, binary? }]` برمی‌گردد، بنابراین زنجیره‌سازی با هر ابزار بیرونی طبیعی است.

سه قابلیتی که یک کلاینت بیرونی روی آن حساب می‌کند:

- **حالت همگام:** `?wait=true` تا `RUN_WAIT_MAX_MS` صبر می‌کند و نتیجه را inline
  برمی‌گرداند؛ در timeout پاسخ `202` با `pollUrl` می‌دهد.
- **Idempotency:** هدر `Idempotency-Key` روی `POST /run` تضمین می‌کند تلاش دوباره
  با همان کلید، همان جاب قبلی را برگرداند و جاب تکراری ایجاد نشود.
- **Webhook امضاشده:** با ست‌کردن `WEBHOOK_SECRET`، بدنهٔ خروجی با
  HMAC-SHA256 امضا می‌شود (`X-Signature`, `X-Webhook-Timestamp`).

مشخصات ماشین‌خوان در [`openapi.yaml`](./openapi.yaml) است.
