...
- [x] **استپ ۲۶ — اجرای زنده + نتیجهٔ هر نود مثل n8n + پنل لاگ همیشه‌جلوی‌چشم**
  1. ✅ **رِدیوسرِ DOM-free وضعیت اجرا** `public/js/run-state.js` (`window.RunState = {create,reset,applyEvent,stepAt,stepStatus,nodeStatusMap,counts,isTerminal}`): جریان رویدادهای زنده (`job.start`/`step.start`/`step.done`/`step.error`/`job.error`/`log`) را به وضعیتِ آمادهٔ رندر تبدیل می‌کند؛ ایندکس استپِ بک‌اند (۱-مبنا) را به ایندکس نودِ زنجیره (۰-مبنا = `index-1`) نگاشت می‌کند؛ `step.done` با `success:false` را خطا می‌گیرد؛ سقف لاگ `LOG_CAP=500`؛ در برابر رویداد null/خارج‌ازترتیب/تکراری مقاوم (هرگز throw نمی‌کند).
  2. ✅ **پنل لاگ/اجرا** `public/js/run-panel.js` (`window.RunPanel = {mount,unmount,open,close,toggle,startJob,stop,loadLastRun,pin,unpin,getPins}`): کشوی پایینِ قابل‌جمع‌شدن (singleton روی `<body>`) با خط زمانی استپ‌ها (آیکن وضعیت + شماره + اکشن + تعداد آیتم/زمان/علتِ خطا)، badge وضعیت کلی و وضعیت اتصال؛ متصل به `window.LiveClient` (WS با fallbackِ SSE).
  3. ✅ **روی نود (در `flow-editor.js`):** هالهٔ «در حال اجرا» (status-running) سپس **badge سبز(✓ + تعداد آیتم + زمان)** یا **قرمز(✕ + علت)** با **tooltip**؛ متدهای جدید `setNodeResultsByIndex(chainIndex0, {output, meta})` و `selectByChainIndex(chainIndex0)` و `pinByIndex/isPinnedByIndex` که ایندکس ۰-مبنای زنجیره را از طریق `chainNodeIds()` به `nodeId` نگاشت می‌کنند.
  4. ✅ **کلیک روی نود/ردیف خط‌زمان → OUTPUT آن استپ** در NDV (آیتم‌های خروجی + متادیتا)؛ **ذخیرهٔ «آخرین اجرا»ی هر ورکفلو** در `localStorage` (`ab_last_run:<wfId>`) برای نمایش پس از reload؛ **Pinning** خروجی نود (دکمهٔ 📌 در خط‌زمان + نشانگرِ روی کارتِ نود).
  - ✅ تست DOM-free `tests/unit/run-state.test.ts` (**۱۴ تست** via `node:vm` با شیمِ window): چرخهٔ عمر، شمارش آیتم، خطاها (`step.error`/`success:false`/fallbackِ message/`job.error`)، مقاومت (null/خارج‌ازترتیب/dedup/سقف لاگ ۵۰۰/`reset` که `jobId` را نگه می‌دارد)، و نگاشت ۱-مبنا→۰-مبنا. سیم‌کشی اجرای زنده در `views.js` (#fe-run → `RunPanel.startJob` + mount + load/persist last-run + unmount هنگام ترک ویو). i18n کامل fa+en: ۱۱ کلید جدید `rp.*` با برابری **۳۳۳ کلید** (همه count=2). CSS: کشوی `.run-panel`/`.rp-*` + badgeهای روی نود + نشانگر pin. ترتیب لود اسکریپت: …→live→**run-state→run-panel**→browser-view→views. `tsc`/`build` سبز، `npm test`=**۲۵۱** (۲۳۷+۱۴)، smoke مرورگر=۰ خطای کنسول. (هیچ تغییری در `src/**` نبود — بک‌اند از قبل همهٔ رویدادها/فیلدهای step.done را می‌فرستد؛ صرفاً UI/CSP-safe. بدون اسکریپت throwaway.)

- [x] **استپ ۲۷ — Error handling حرفه‌ای (هم‌تراز n8n)** ✅ _(۲۰۲۶-۰۶-۰۶)_
  1. ✅ هر نود: گزینه‌های **Continue On Fail** و **Retry On Fail** (تعداد دفعات + فاصله) در بخش **Settings** پنل NDV (روی `node.errorPolicy`؛ سریالایز به فیلدهای top-level استپ: `continueOnFail`/`retryOnFail`/`maxTries`/`waitBetweenTriesMs`). موتور: `src/core/ErrorPolicy.ts` (هستهٔ خالص، بدون side-effect: `normalizeErrorPolicy`/`shouldRetry`/`retryDelayMs` با backoff خطی + سقف؛ کلمپ مقادیر؛ تحملِ ورودی null/garbage). حلقهٔ اجرای استپ در `pipeline.ts` با **labelled `stepLoop:` + retry `while`** بازنویسی شد (هر ۴۰ `continue;` بدنه به `continue stepLoop;` تبدیل شد تا با حلقهٔ retry تداخل نکند)؛ هنگام خطا: rollback شمارنده‌ها → سعی مجدد (`step.retry` event) یا continue-on-fail (ثبت خطا + ذخیرهٔ `{error}` در saveAs + ادامه) یا rethrow؛ خطاهای کنترلیِ کشنده (CANCELLED/PAGE_CLOSED/QUOTA/Safety/WorkflowFail) هرگز retry/swallow نمی‌شوند.
  2. ✅ دادهٔ استاندارد **Error Trigger** با `buildErrorTriggerData` (شکل n8n: `execution.error.{message,stack,name}`, `lastNodeExecuted`, `workflow.{id,name}`, `mode`) — آمادهٔ اتصال به Error Workflow در استپ‌های بعدی.
  3. ✅ نود **Stop And Error** (`stop_and_error`) برای شکست عمدی تحت شرط (پیام expressionable).
  - ✅ تست‌های DOM-free: `tests/unit/error-policy.test.ts` (**۲۴ تست**: normalize/clamp، retry/backoff، Stop-And-Error، شکل دادهٔ Error Trigger) + ۴ تست round-trip در `graph-serialize.test.ts`. i18n کامل fa+en (۸ کلید `settings.*`/`help.*`، برابری **۳۴۱ کلید**). CSS بخش Settings (RTL-aware). `tsc`/`build` سبز، `npm test`=**۲۷۹** (۲۵۱→۲۷۹)، smoke مرورگر=۰ خطای کنسول. خطوط: `src/**`=CRLF (ویرایش با اسکریپت Python باینری، حذف‌شده)، `public/**`+`tests/**`=LF.

- [x] **استپ ۲۸ — Triggerها (مدل ب برای n8n) + موتور فعال‌سازی با تزریق دیتا** ✅ _(۲۰۲۶-۰۶-۰۶)_
  1. ✅ **هستهٔ خالص** `src/core/TriggerEngine.ts` (CRLF): `normalizeTrigger` (تشخیص نوع manual/webhook/schedule/telegram از `action`/`kind` + aliasها، uppercase متد، trim cron/timezone، گزارش `hasToken` بدون افشای توکن)، `isTriggerAction`، `buildItemsFromTrigger` (نگاشت `body/headers/query/method` ورودی → `items[].json`؛ آبجکت→یک آیتم با spread کلیدها + envelope؛ آرایه→یک آیتم به‌ازای هر عنصر؛ primitive→`{value}`؛ تحمل null)، `buildManualItems` (دادهٔ دستی→آیتم‌ها)، `validateCron` (۵–۶ بخش)، `verifyTriggerAuth` (HMAC با reuse از `signature.ts` + توکن length-checked؛ هر دو در صورت تنظیم باید پاس شوند؛ بدون تنظیم=باز).
  2. ✅ **تزریق دیتا در موتور اجرا:** `pipeline.ts` پارامتر اختیاری `initialItems` گرفت → اگر موجود و غیرخالی باشد جایگزین `emptyStream()` می‌شود؛ نودهای trigger در بدنهٔ اجرا **no-op pass-through** هستند (دیتا قبلاً در worker تزریق شده). `index.ts` worker از `job.data.triggerData` (شکل `{source,body,headers,query,method}` یا `{data}`) آیتم‌های اولیه را می‌سازد و به `runPipeline` می‌دهد. `/run` و `/workflows/:id/run` و `schemas.ts` فیلد اختیاری `triggerData` را می‌پذیرند و به جاب فوروارد می‌کنند. **Schedule/Cron از قبل در بک‌اند کامل بود** (BullMQ repeatable: `POST /schedule`، `GET /schedules/:userId`، `DELETE /schedule/...` + سقف schedule per-plan).
  3. ✅ UI: ۴ نود trigger در `actions.js` (دستهٔ `trigger`): `trigger_manual` (فیلد `data` JSON/expr)، `trigger_webhook` (`method`/`path`/`secret`)، `trigger_schedule` (`cron`/`timezone`)، `trigger_telegram` (`botToken`/`chatId`) + export `TRIGGER_IDS`/`isTrigger`. i18n ۱۶ کلید جدید fa+en (`p.*`/`help.*`، برابری **۳۶۵=۳۶۵**).
  - ✅ تست‌ها: `tests/unit/trigger-engine.test.ts` (**۳۲ تست** LF: normalize/isTriggerAction/buildItemsFromTrigger/buildManualItems/validateCron/verifyTriggerAuth شامل امنیت HMAC+توکن) + ۳ تست trigger در `action-catalog.test.ts` (→**۱۲**). `tsc`/`build` سبز، `npm test`=**۳۱۵** (۲۸۰→۳۱۵)، smoke مرورگر=۰ خطای کنسول. خطوط: `src/**`=CRLF (TriggerEngine.ts با اسکریپت Python باینری، حذف‌شده؛ `schemas.ts` تاریخاً LF بود و حفظ شد)، `public/**`+`tests/**`=LF.

- [x] **استپ ۲۹ — گزارش لحظه‌ای دوکاناله به بیرون (Webhook هر استپ + لینک مشاهدهٔ زنده)** ✅ _(۲۰۲۶-۰۶-۰۶)_
  1. ✅ **هستهٔ خالص** `src/core/StepReporter.ts` (CRLF، DOM-free/بدون شبکه): `isStepEventType`/`STEP_EVENT_TYPES` (فقط `step.start/done/error/retry`)، `buildStepWebhookPayload` (envelope تخت و n8n-friendly از eventهای زندهٔ Step 16/21: `event,jobId,userId,index,action,success?,durationMs?,inputItemCount?,outputItemCount?,outputSample?,outputTruncated?,error?,attempt?,maxTries?,timestamp`؛ برای رویدادهای غیر-step مقدار `null`)، `shouldDeliverStepEvent` (allow-list اختیاری، هم‌تراز multiOptions نود n8n)، و توکن **لینک اشتراکی** امضاشده: `buildShareToken`/`verifyShareToken` (`<b64url(userId)>.<b64url(jobId)>.<exp>.<hmacHex>`، HMAC-SHA256 روی `userId:jobId:exp`، مقایسهٔ constant-time، انقضای تزریق‌پذیر، سقف TTL).
  2. ✅ **Webhook خروجی per-step (کانال ۱):** در `index.ts` داخل `onEvent` ورکر، وقتی `STEP_WEBHOOK_ENABLED=true` و `webhookUrl` ست باشد، هر رویداد استپ با `sendStepWebhook` (در `webhook.service.ts`؛ همان HMAC `X-Signature`+`X-Webhook-Timestamp` و retry/backoff job webhook + هدر `X-Webhook-Event`) به‌صورت fire-and-forget ارسال می‌شود (هرگز داخل pipeline throw نمی‌کند).
  3. ✅ **نود Trigger n8n** گسترش یافت: چهار رویداد `step.start/step.done/step.error/step.retry` به multiOptions «Events» اضافه شد (هم‌چنان با HMAC verify و event-filter موجود).
  4. ✅ **لینک مشاهدهٔ زندهٔ قابل‌اشتراک (کانال ۲):** مسیر `POST /live/share/:userId/:jobId` (نیازمند احراز API-key مالک) توکن امضاشده + URL کامل می‌سازد؛ `GET /live/view/:userId/:jobId` صفحهٔ فقط‌خواندنی `public/live-view.html`+`public/js/live-view.js` (مستقل، CSP-safe، RTL) را سرو می‌کند که با همان توکن `share` به `/live/ws`↔`/live/sse` وصل می‌شود و نودبه‌نود + خلاصهٔ خروجی هر استپ را نمایش می‌دهد. `authorizeLive` گسترش یافت تا توکن `share` معتبر بسته به `(userId, jobId)` را بپذیرد (در صورت نامعتبر بودن، fall-through به احراز API-key). کلید امضا: `LIVE_SHARE_SECRET` با fallback به `WEBHOOK_SECRET`→`API_TOKEN`.
  - ✅ تست‌ها: `tests/unit/step-reporter.test.ts` (**۲۴ تست** LF: isStepEventType/shouldDeliver/buildStepWebhookPayload برای هر چهار رویداد + امنیت و round-trip توکن اشتراکی شامل secret/jobId/userId اشتباه، انقضا، malformed، clamp TTL) + **۶ تست** authorize در `tests/integration/live-auth.test.ts` (share معتبر بدون API-key، عدم‌تطابق userId/jobId، secret اشتباه، برتری share بر API-key نامعتبر). `tsc`/`build` سبز، `npm test`=**۳۴۵** (۳۱۵→۳۴۵، ۲۴ فایل)، smoke مرورگر index.html + live-view.html = ۰ خطای کنسول. i18n کامل fa+en (۸ کلید `live.*`، برابری **۳۶۵=۳۶۵**). خطوط: `src/**`=CRLF (StepReporter.ts/config.ts/LiveServer.ts/webhook.service.ts/index.ts با اسکریپت Python باینری، حذف‌شده)، `public/**`+`tests/**`+`n8n-node/**`=LF.

- [x] **استپ ۳۰ — نود n8n غنی (هم‌تراز فیلدهای پنل)** ✅ _(۲۰۲۶-۰۶-۰۶)_
  1. ✅ ارتقای `AutomationBackend.node.ts`: عملیات جدید **Run Saved Workflow** (پیش‌فرض) با انتخاب ورکفلوی ذخیره‌شده از dropdown که با **`loadOptions` (`getWorkflows`)** زنده از `GET /workflows/:userId` پر می‌شود (وابسته به `userId` با `loadOptionsDependsOn`)؛ تزریق **Trigger Data (JSON)** به‌عنوان آیتم‌های ورودی اولین نود، override اختیاری `headless`/`webhookUrl` فقط برای همان اجرا، حالت sync (`?wait=true`)/async و هدر `Idempotency-Key` (هم‌قرارداد با `/run`). عملیات قدیمی به **Run Inline Workflow** تغییرنام یافت (همان `value:'run'`).
  2. ✅ عملیات اصلی **مدل ب** «اجرای ورکفلوی ذخیره‌شده با این دیتا» → `POST /workflows/:userId/:workflowId/run`. منطق ساخت درخواست در تابع خالصِ **export‌شدهٔ** `buildRequestOptions(baseUrl, params)` (بدون side-effect/بدون نیاز به runtime n8n) جدا شد تا قرارداد payload مستقیماً تست شود. نمونه‌ورکفلوی جدید `n8n-node/examples/run-saved-workflow.json` (Schedule Trigger → Run Saved Workflow(wait) → Trigger webhook).
  3. ✅ ارتقای type-shim محلی `types/n8n-workflow.d.ts`: افزودن `ILoadOptionsFunctions` (+`httpRequest`)، `INodePropertyCollection`، `methods.loadOptions` در `INodeType`، و `hint`/options ترکیبی. (پکیج standalone با `tsc -p` و `noUnusedLocals` سبز کامپایل/build شد؛ dist پاک شد.)
  - ✅ تست قرارداد: `tests/unit/n8n-node.test.ts` (**۱۹ تست** LF) با mock کردن `n8n-workflow` (فقط دو کلاس خطا) — تطبیق دقیق payload با هر ۵ endpoint (runSaved/run/schedule/getJob/cancel) + `parseTriggerData` (object/blank/invalid) + `normalizeBase` + قرارداد مشترک `?wait`+`Idempotency-Key`. `npm test`=**۳۶۴** (۳۴۵→۳۶۴، ۲۵ فایل)، `tsc` اصلی + `tsc -p n8n-node` سبز. خط‌پایان: `n8n-node/**`+`tests/**`=LF (با Edit/Write، `src/**` دست نخورد).

- [x] **استپ ۳۱ — افزونهٔ کروم = همان پنل (مدل الف) + Element Picker + تیک/خطا روی نود** ✅ _(۲۰۲۶-۰۶-۰۷)_
  1. ✅ افزونه با **همان API_TOKEN** به سرور وصل می‌شود و **همان ورکفلوهای ذخیره‌شده** را نشان می‌دهد (نه کپی موازی): دکمهٔ **Test connection** علاوه بر `GET /me`، **userId کانونیکال** کلید را resolve و auto-fill می‌کند (در self-hosted = `local`)، و کارت **Workflows** فهرست `GET /workflows/:userId` را با name·version·stepCount رندر می‌کند؛ دکمهٔ **Open panel** همان UI داشبورد را در تب جدید باز می‌کند (`buildPanelUrl`). دکمهٔ **▶ Run** هر ورکفلو → `POST /workflows/:userId/:workflowId/run` (مدل ب، هم‌قرارداد با نود n8n)، با تیکِ **show browser** برای اجرای headful.
  2. ✅ **Element Picker**: همان `selector.js` (`window.ABSelector` → cssPath/xPath پایدار، هم‌تراز picker سمت سرور) → دکمه‌های **+ click step** / **+ extract step**؛ extract اکنون **نامِ فیلد را prompt** می‌کند و selector را مستقیماً در همان فیلد نود درج می‌کند (مزیت اصلی افزونه).
  3. ✅ **کانال زنده**: پس از اجرا، service worker جریان `GET /live/sse/:userId/:jobId?api_key=…` را با `fetch`+`ReadableStream` (در MV3 worker `EventSource` نیست) می‌خواند، هر رویداد را با `ABCore.mapLiveEventToStatus` به یک delta نگاشت و به popup forward می‌کند؛ popup روی هر نود **✓/✗/↻** می‌کشد (هم‌راستا با استپ ۲۶) و با `isTerminalEvent` استریم را می‌بندد. اجرای مستقیم روی تب فعلی هم با همان flow inline (`POST /run`) + استریم زنده پشتیبانی می‌شود.
  - ✅ **ماژول هستهٔ خالص مشترک** `extension/lib/ab-core.js` (`window.ABCore` + `module.exports`، بدون DOM/chrome): `normalizeBase`/`buildRunSavedUrl`/`buildRunInlineUrl`/`buildSseUrl`/`buildPanelUrl`/`parseWorkflowList`/`resolveUserId`/`mapLiveEventToStatus`/`isTerminalEvent`/`stepLabel`/`extractJobId` — یک منبع حقیقت که هم popup، هم background (`importScripts`) و هم تست‌ها مصرف می‌کنند.
  - ✅ تست‌ها: `tests/unit/ab-core.test.ts` (**۳۲ تست** LF) با همان الگوی `node:vm` + sandboxِ `window` ساختگی (مثل `extension-selector.test.ts`): URL builderها (encode، `?wait`، `?api_key`)، `parseWorkflowList` (آرایه/`{workflows}`/junk/fallback name→id)، `resolveUserId`، نگاشت کامل live-event (job/step start/done/error/retry + flat/wrapped + log/unknown→null)، `isTerminalEvent`، `stepLabel` همهٔ اکشن‌ها، `extractJobId`. تست موجود `extension-selector.test.ts` همچنان سبز. `npm test`=**۳۹۶** (۳۶۴→۳۹۶، ۲۶ فایل)، `tsc` اصلی + `tsc -p n8n-node` سبز. خط‌پایان: `extension/**`+`tests/**`=LF (با Write/Edit، `src/**` دست نخورد). بدون نیاز به web_accessible_resources (popup/worker هر دو extension-origin).

- [x] **استپ ۳۲ — همگام‌سازی کاتالوگ اکشن + Templates + جمع‌بندی/مستندسازی** ✅ _(۲۰۲۶-۰۶-۰۷)_
  1. ✅ **Single Source of Truth:** `public/js/actions.js` با فهرست اکشن‌های `src/pipeline.ts` همگام شد (≈۱۶ اکشن کانونیکال افزوده شد: dblclick/focus/mouse-move/drag-drop/check/uncheck/download/attribute/http-request/remove-element/add-style/switch-frame/switch-tab/close-tab/close-browser/handle-dialog). **guard-test** در `tests/unit/action-catalog.test.ts` (بلوک «Step 32 backend/UI sync»، **۳ تست**): `pipeline.ts` را با regex پارس می‌کند (`step.action === '...'` + `[...].includes(step.action)`) و تضمین می‌کند **هر اکشنی که بک‌اند dispatch می‌کند** از کاتالوگ UI قابل‌دسترسی است — مستقیم یا از طریق نقشهٔ `ALIASES` (مثل goto/goto-url/navigate)؛ کلمات کنترلیِ صرفاً runtime (break/continue/return/fail) در `BACKEND_ONLY` معاف‌اند. (بک‌اند allowlist ندارد؛ این تست جای آن را به‌عنوان گارد توسعه می‌گیرد.)
  2. ✅ **Templates:** `public/js/templates.js` (`window.TEMPLATES`، خالص بدون DOM/fetch) با **۳ الگوی آماده** — `price-scrape` (اسکرپ قیمت)، `login-form` (پرکردن فرم پشت لاگین، رمز via expression)، `scheduled-screenshot` (تریگر زمان‌بندی روزانه + اسکرین‌شات). دکمهٔ **🧩 Templates** + پیکر در `views.js`(`renderTemplates`) → با یک کلیک به‌عنوان ورکفلوی جدید (`API.createWorkflow`) ذخیره می‌شود (deep-copy، بدون نشت به الگو). تست `tests/unit/templates.test.ts` (**۹ تست**) شکل کاتالوگ، یکتایی idها، کلیدهای i18n (`tpl.*`)، deep-copy در `toWorkflowBody`، و **حضور هر اکشنِ الگو در کاتالوگ اکشن** را اعتبارسنجی می‌کند. کلیدهای i18n الگوها به fa+en افزوده شد (parity حفظ شد).
  3. ✅ **راهنمای end-to-end:** `docs/END_TO_END_GUIDE.md` (فارسی، هم‌سبک API.md) — نقشهٔ کلی، دو مدل اجرا (A=inline `/run`، B=ورکفلوی ذخیره `/workflows/:u/:w/run`)، سه نقطهٔ ورود (n8n/پنل/افزونه) روی **یک موتور واحد** (`pipeline.ts`)، کانال زنده WS+SSE (با `?api_key=` برای افزونهٔ MV3)، Templates، Single Source of Truth، و جدول عیب‌یابی. **بازبینی دسترس‌پذیری:** `:focus-visible` سراسری (چون `.field-input:focus{outline:none}` بود)، utility `.sr-only`، و `@media (prefers-reduced-motion: reduce)` به `styles.css` افزوده شد؛ ریسپانسیو (۶۴۰/۸۲۰/۹۸۰/۱۱۰۰px) و تم (`:root`+`[data-theme=light]`) و RTL/`lang=fa` از قبل موجود بودند.
  - ✅ تست‌ها: `npm test`=**۴۰۸** (۳۹۶→۴۰۸، ۲۷ فایل)، `tsc` اصلی + `tsc -p n8n-node` سبز. خط‌پایان: `public/**`+`tests/**`+`docs/**`=LF.
