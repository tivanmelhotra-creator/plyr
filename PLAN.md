...
- [ ] **استپ ۲۴ — نودهای شاخه‌دار و سریالایز کامل گراف غیرخطی**
  ...
  2. سریالایز گراف غیرخطی به `steps[]` تو‌در‌توی بک‌اند (then/else/cases/catch/finally) و دیسریالایز معکوس.
  3. اعتبارسنجی گراف (نود بی‌اتصال، حلقهٔ بی‌پایان، trigger الزامی در ابتدا).
  - تست سریالایز/دیسریالایز شاخه‌دار + تست اعتبارسنجی.

- [ ] **استپ ۲۵ — NDV (Node Detail View) سه‌ستونه + فیلدهای غنی + Expression/Mapping**
  1. پنل سه‌ستونه: **INPUT** (آیتم‌های ورودی به‌صورت Table/JSON/Schema) | **Parameters** | **OUTPUT** (نتیجهٔ همان نود).
  2. catalog فیلد ارتقا یابد به: string/password/multiline، number(min/max)، **boolean(toggle)**، options/multiOptions، **collection/fixedCollection**، **assignmentCollection (نام=مقدار)**، dateTime، **code(JS)**، **json**، **filter (شرط‌ساز)**.
  3. **Expression engine سبک** (`{{ $json.x }}`, `$node["n"].json`, `$now`, توابع پایه) + حالت Fixed/Expression هر فیلد + **drag&drop از INPUT** که اکسپرشن می‌سازد.
  4. help/description هر فیلد + اعتبارسنجی inline. ⚠️ امنیت: ارزیابی Expression در **sandbox محدود** (نه `eval` خام؛ درس از CVE اخیر n8n).
  - تست ارزیابی Expression + map به params بک‌اند + تست امنیت sandbox.
